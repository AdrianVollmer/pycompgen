...
from pycompgen.models import PackageManager, CompletionType
...
class TestMainWorkflow:
    ...
    def test_main_workflow_success(
        ...
        mock_save_completions.assert_called_once_with(mock_completions, temp_dir, force=False)
...
class TestEndToEndWorkflow:
    ...
    def test_e2e_no_packages_found(self, mock_run, temp_dir, capsys):
        ...
        with patch('sys.argv', ['pycompgen', '--cache-dir', str(temp_dir), '--verbose']):
            main()
    ...
    def test_e2e_click_package_with_completion(self, mock_exists, mock_run, temp_dir, capsys):
        """Test workflow with click package that supports completions."""
        # Mock uv tool list output
        uv_output = "click-package v1.0.0 (path: /fake/venv)\n"
        ...
        with tempfile.TemporaryDirectory() as temp_venv:
            ...
            mock_run.side_effect = [
                # uv tool list
                Mock(stdout=f"click-package v1.0.0 (path: {venv_path})\n", returncode=0),
                # pipx list (fails)
                subprocess.CalledProcessError(1, ["pipx"]),
            ...
            with patch('pycompgen.analyzers.find_package_commands', return_value=['click-command']):
                with patch('sys.argv', ['pycompgen', '--cache-dir', str(temp_dir), '--verbose']):
                    main()
    ...
    def test_e2e_pipx_argcomplete_package(self, mock_run, temp_dir, capsys):
        ...
        with patch('pathlib.Path.exists', return_value=True):
            with patch('pycompgen.analyzers.find_package_commands', return_value=['argcomplete-command']):
                with patch('sys.argv', ['pycompgen', '--cache-dir', str(temp_dir), '--verbose']):
                    main()
...
class TestErrorScenarios:
    ...
    @patch('pycompgen.detect_packages')
    @patch('pycompgen.setup_logging')
    def test_analysis_error_handling(self, mock_setup_logging, mock_detect, mock_analyze):
    ...
    @patch('pycompgen.detect_packages')
    @patch('pycompgen.setup_logging')
    def test_generation_error_handling(self, mock_setup_logging, mock_detect, mock_analyze, mock_generate):
