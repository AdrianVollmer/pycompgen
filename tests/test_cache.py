...
class TestGetCacheDir:
    ...
    def test_get_cache_dir_xdg_cache_home(self):
        """Test cache directory with XDG_CACHE_HOME set."""
        result = get_cache_dir()
        
        assert result == Path("/custom/cache/pycompgen")

    @patch.dict(os.environ, {}, clear=True)
    @patch('pathlib.Path.home')
    def test_get_cache_dir_fallback(self, mock_home):
        """Test cache directory fallback to ~/.cache."""
        mock_home.return_value = Path("/home/user")
        
        result = get_cache_dir()
        
        assert result == Path("/home/user/.cache/pycompgen")

    @patch.dict(os.environ, {"XDG_CACHE_HOME": ""})
    @patch('pathlib.Path.home')
    def test_get_cache_dir_empty_xdg(self, mock_home):
...
class TestGenerateSourceScript:
    ...
    def test_generate_source_script_nonexistent_directory(self):
...
class TestIntegrationWithMockEnv:
    ...
    def test_full_cache_workflow(self, mock_glob, mock_write, mock_exists, mock_mkdir):
        """Test complete cache workflow with mocked file operations."""
        # Setup mocks
        mock_exists.return_value = False  # Directory doesn't exist
        mock_glob.return_value = [Path("/test/cache/pycompgen/package1.sh")]
        ...
        assert cache_dir == Path("/test/cache/pycompgen")
